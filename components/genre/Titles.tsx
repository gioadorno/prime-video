	...
	return (titles.length > 0 &&
		<section className="block w-full z-0 mb-9 ">
			<div className="flex items-center ml-[4%] justify-start pb-2 gap-3">
				<span className="text-2xl font-serif text-[#1a98ff] font-bold">Prime</span>
				<h2 className="text-white font-serif font-bold text-xl">{ category }</h2>
			</div>
			<div
			...
			>
				...
				<button
					...
					className={ `${ showLeftButton && showIcons
						? "cursor-pointer"
						: "cursor-default"
						} left scroll-button text-white group pr-8 z-20` }
				>
					<ChevronLeftIcon
						className={ `${ showLeftButton && showIcons ? "block" : "hidden"
							} group-hover:scale-150 duration-150 ease-in` }
						width={ 24 }
						height={ 24 }
					></ChevronLeftIcon>
				</button>
				<ul className={ `genre-container no-scrollbar gap-4 mx-auto relative max-w-[92%]` } ref={ wrapperRef }>
					{ titles.length > 0 &&
					...
					<li onClick={ () => routeTo({
						...
						type,
						path: 'genre'
					}) } id={ `${ category + titles.length }` } className={ `list-title rounded-xl cursor-pointer bg-slate-600/60 hover:bg-slate-500/60 duration-150 ease-in group w-[350px] ml-2` }>
						<article className="carousel-card flex items-center justify-center flex-wrap h-[150px] md:h-[150px] max-w-[250px] md:max-w-[350px] relative">
							<p className="text-white text-xl group-hover:scale-110 duration-150 ease-in">View more { category }</p>
						</article>
					</li>
				</ul>
				{/* Next */ }
				<button
					...
					className={ `${ showRightButton && showIcons
						? "cursor-pointer"
						: "cursor-default"
